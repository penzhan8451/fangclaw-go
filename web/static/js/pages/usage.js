...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await FangClawGoAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await FangClawGoAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await FangClawGoAPI.get('/api/usage/daily');
