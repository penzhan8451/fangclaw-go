// FangClaw-go Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    startStreaming: function() {
      ...
      var sep = '?';
      var token = FangClaw-goAPI.getToken();
    ...
    async fetchLogs() {
      ...
      try {
        var data = await FangClaw-goAPI.get('/api/audit/recent?n=200');
    ...
    async verifyChain() {
      try {
        var data = await FangClaw-goAPI.get('/api/audit/verify');
