...
func (m *BrowserManager) Scroll(agentID, direction string, amount int) (string, error) {
	resp, err := m.sendCommand(agentID, BrowserCommand{
		Action:    "Scroll",
		Direction: direction,
		Amount:    amount,
	...
	return fmt.Sprintf("Scrolled %s by %d pixels. Position: (%.0f, %.0f)", data.Direction, data.Amount, data.ScrollX, data.ScrollY), nil
}

// Wait waits for an element to appear on the page
func (m *BrowserManager) Wait(agentID, selector string, timeoutMs int) (string, error) {
	resp, err := m.sendCommand(agentID, BrowserCommand{
		Action:    "Wait",
		Selector:  selector,
		TimeoutMs: timeoutMs,
...
// RunJs executes JavaScript in the browser
func (m *BrowserManager) RunJs(agentID, expression string) (interface{}, error) {
	resp, err := m.sendCommand(agentID, BrowserCommand{
		Action:     "RunJs",
...
func (m *BrowserManager) Back(agentID string) (string, error) {
	resp, err := m.sendCommand(agentID, BrowserCommand{
		Action: "Back",
	})
	if err != nil {
...
def handle_command(page, context, action, cmd, timeout_ms):
    if action == "Navigate":
        ...
        return {"success": True, "data": {"scrollX": scroll_x, "scrollY": scroll_y, "direction": direction, "amount": amount}}

    elif action == "Wait":
        selector = cmd.get("selector", "")
        timeout = cmd.get("timeout_ms", timeout_ms)
        if not selector:
            return {"success": False, "error": "Missing 'selector' parameter"}
        try:
            page.wait_for_selector(selector, timeout=timeout)
            return {"success": True, "data": {"found": True, "selector": selector}}
        except Exception as e:
            return {"success": False, "error": f"Timed out waiting for selector: {selector} ({timeout}ms)"}
        ...
        page.wait_for_load_state("domcontentloaded", timeout=timeout_ms)
        title = page.title()
        return {"success": True, "data": {"title": title, "url": page.url}}
