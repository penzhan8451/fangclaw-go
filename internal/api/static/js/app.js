...
document.addEventListener('alpine:init', function() {
  ...
  Alpine.store('app', {
    ...
    async refreshAgents() {
      try {
        var agents = await FangClawGoAPI.get('/api/agents');
    ...
    async checkStatus() {
      try {
        var s = await FangClawGoAPI.get('/api/status');
        this.mainPollFailCount = 0;
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        this.mainPollFailCount++;
        if (this.mainPollFailCount >= this.maxPollFailures) {
          console.log('[FangClaw-Go] Too many status check failures, stopping polling');
    ...
    refreshPendingApprovalsCount() {
      ...
      FangClawGoAPI.get('/api/approvals').then(function(response) {
        ...
        self.approvalPollFailCount++;
        if (self.approvalPollFailCount >= self.maxPollFailures) {
          console.log('[Approvals] Too many failures, stopping polling');
...
function app() {
  return {
    ...
    init() {
      ...
      FangClawGoAPI.onConnectionChange(function(state) {
        Alpine.store('app').connectionState = state;
      });

      // Page visibility change listener - restart polling when page becomes visible
      document.addEventListener('visibilitychange', function() {
        if (document.visibilityState === 'visible') {
          var store = Alpine.store('app');
          if (store.currentUser) {
            console.log('[FangClaw-Go] Page visible, restarting polling...');
            store.mainPollFailCount = 0;
            store.approvalPollFailCount = 0;
            // Restart main polling if not running
            if (!store.pollIntervalId) {
              store.pollIntervalId = setInterval(function() { self.pollStatus(); }, 10000);
            }
            self.pollStatus();
            // Restart approval polling if not running
