  ...
  return {
    ...
    generateToml() {
      ...
      if (f.profile && f.profile !== 'custom') {
        ...
        var profileMatch = this.spawnProfiles.find(function(p) { return p.name === f.profile; });
        if (profileMatch && profileMatch.tools && profileMatch.tools.length > 0) {
          lines.push('', 'tools = [');
          for (var i = 0; i < profileMatch.tools.length; i++) {
            var tool = profileMatch.tools[i];
            lines.push('  "' + tool.replace(/"/g, '\\"') + '"' + (i < profileMatch.tools.length - 1 ? ',' : ''));
          }
          lines.push(']');
        }
      }
