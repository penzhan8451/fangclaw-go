// FangClaw-go Chat Page — Agent chat with markdown + streaming
'use strict';
...
function chatPage() {
  ...
  return {
    ...
    executeSlashCommand(cmd, cmdArgs) {
      ...
      // 尝试通过 WebSocket 发送命令到后端
      var trySendCommandWs = function(command, args) {
        if (self.currentAgent && FangClawGoAPI.isWsConnected()) {
          var wsPayload = { type: 'command', data: { command: command, args: args || '' } };
      ...
      switch (cmd) {
        ...
        case '/new':
        case '/reset':
          if (!trySendCommandWs(cmd, cmdArgs)) {
        ...
        case '/verbose':
          if (!trySendCommandWs(cmd, cmdArgs)) {
            ...
            if (self.currentAgent) {
              ...
              var fallbackText = '';
              if (cmd === '/usage') {
                fallbackText = '**Session Usage**\n- Messages: ' + self.messages.length + '\n- Approx tokens: ~' + approxTokens;
              } else if (cmd === '/context') {
                fallbackText = '**Context Window**\n- Messages: ' + self.messages.length + '\n- Approx tokens: ~' + approxTokens + '\n- Queue size: ' + self.messageQueue.length;
              } else if (cmd === '/queue') {
                fallbackText = '**Queue Status**\n- Sending: ' + (self.sending ? 'Yes' : 'No') + '\n- Pending messages: ' + self.messageQueue.length;
              } else if (cmd === '/verbose') {
                if (!self.verboseMode) self.verboseMode = 'off';
                if (cmdArgs === 'off') {
                  self.verboseMode = 'off';
                } else if (cmdArgs === 'on') {
                  self.verboseMode = 'on';
                } else if (cmdArgs === 'full') {
                  self.verboseMode = 'full';
                } else {
                  if (self.verboseMode === 'off') {
                    self.verboseMode = 'on';
                  } else if (self.verboseMode === 'on') {
                    self.verboseMode = 'full';
                  } else {
                    self.verboseMode = 'off';
                  }
                }
                fallbackText = 'Verbose mode **' + self.verboseMode + '**.';
              }
              if (fallbackText) {
                self.messages.push({ id: ++msgId, role: 'system', text: fallbackText, meta: '', tools: [] });
                self.scrollToBottom();
              }
            }
          }
    ...
    handleWsMessage(data) {
      switch (data.type) {
        ...
        case 'command_result':
          ...
          this.messages.push({ id: ++msgId, role: 'system', text: data.data?.content || data.message || 'Command executed.', meta: '', tools: [] });
          this.scrollToBottom();
          break;
