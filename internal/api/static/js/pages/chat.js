...
function chatPage() {
  ...
  return {
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      // Priority: Use HTTP endpoint first (complete agent runtime)
      this.messages.push({ id: ++msgId, role: 'agent', text: '', meta: '', thinking: true, tools: [], ts: Date.now() });
      this.scrollToBottom();

      console.log('Sending message to agent by http mode');
      try {
        ...
        console.log('HTTP send failed, trying WebSocket:', e);
