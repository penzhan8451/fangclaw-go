...
function handsPage() {
  return {
    ...
    async loadData() {
      ...
      this.loadError = '';
      try {
        var data = await FangClawGoAPI.get('/api/hands');
    ...
    async loadActive() {
      ...
      try {
        var data = await FangClawGoAPI.get('/api/hands/active');
    ...
    async showDetail(handId) {
      try {
        var data = await FangClawGoAPI.get('/api/hands/' + handId);
    ...
    async deactivateFromAvailable(hand) {
      ...
      if (instance) {
        ...
        // Instance not found, possibly out of sync, reload data
        await this.loadData();
    ...
    async installDeps() {
      ...
      try {
        var data = await FangClawGoAPI.post('/api/hands/' + handId + '/install-deps', {});
    ...
    async recheckDeps() {
      ...
      try {
        var data = await FangClawGoAPI.post('/api/hands/' + this.setupWizard.id + '/check-deps', {});
        if (data.requirements && this.setupWizard.requirements) {
          for (var i = 0; i < this.setupWizard.requirements.length; i++) {
            var existing = this.setupWizard.requirements[i];
            for (var j = 0; j < data.requirements.length; j++) {
    ...
    async loadStats(inst) {
      try {
        var data = await FangClawGoAPI.get('/api/hands/instances/' + inst.instance_id + '/stats');
    ...
    async refreshBrowserView() {
      ...
      try {
        var data = await FangClawGoAPI.get('/api/hands/instances/' + id + '/browser');
        if (data.active) {
